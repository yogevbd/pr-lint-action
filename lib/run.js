...
async function validatePrBody(pr) {
  const {changelog, jira, breakingChanges, notesForQA} = pr;
  console.log(pr);
  if (changelog === '') {
    return false;
  }
  if (jira !== '') {
    const jiraURL = `https://jira.wixpress.com/browse/${jira}`;
    const resp = await request(jiraURL, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Host': 'jira.wixpress.com',
        'Accept': '*/*',
      }
    });
