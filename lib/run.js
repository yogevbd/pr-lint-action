...
async function lintBody(id) {
  ...
  if (mandatorySections) {
    mandatorySections.forEach(async (mandatorySection) => {
      const sectionVerification = verifySection(mandatorySection);
      if (!sectionVerification.isValid) {
      //  console.log(`Error: Inserted - ${sectionVerification.section}: ${mandatorySection.message}`);
        await failure(id, {
          title: mandatorySection.message,
          summary: mandatorySection.message,
          text: `Error: ${mandatorySection.message} (You wrote: ${sectionVerification.section})`,
        });
      }
    });
  }
...
function verifySection(section) {
  ...
  return {isValid: section.validate(result), section: result};
}
